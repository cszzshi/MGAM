...
class AnchorHead(BaseDenseHead):
    ...
    def _get_targets_single(self,
        ...
        if len(pos_inds) > 0:
            ...
            # for i in pos_inds:
            #     weight = 1 + torch.log10(torch.sqrt(torch.mean(gt_areas) /
            #                                         gt_areas[assigned_gt_inds[i] - 1]))
            #     bbox_weights[i, :] = weight
            #     pos_weights = torch.concat([pos_weights, torch.tensor([weight]).cpu()])
        ...
        if len(neg_inds) > 0:
            ...
            # if len(pos_inds) > 0:
            #     label_weights[neg_inds] = torch.mean(pos_weights)
            # else:
            #     label_weights[neg_inds] = 1.0
