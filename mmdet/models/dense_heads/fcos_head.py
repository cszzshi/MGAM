...
class FCOSHead(AnchorFreeHead):
    ...
    # bbox_targets : 每一个先验点的 (x, y)到每一个 gt 的左 右 上 下的距离
    def _get_targets_single(
        ...
        # bbox_targets : [num_points, num_gts, 4]
        ...
        # 绘制正样本数量随面积的变化图
        # global count
        # global anchor_in_per_gt_list
        # global gt_areas_list
        # gt_areas_list = gt_areas_list.cuda()
        # count = count + 1
        # gt_areas = (gt_bboxes_ori[:, 2] - gt_bboxes_ori[:, 0] + 1) * \
        #            (gt_bboxes_ori[:, 3] - gt_bboxes_ori[:, 1] + 1)
        # gt_areas_list = torch.concat([gt_areas_list, gt_areas])
        # anchor_in_per_gt = inside_gt_bbox_mask.sum(0)
        # anchor_in_per_gt_list = torch.concat([anchor_in_per_gt_list, anchor_in_per_gt.cpu()])
        # if count % 6471 == 0:
        #     plt.scatter(gt_areas_list.cpu(), anchor_in_per_gt_list)
        #     plt.show()

        # if there are still more than one objects for a location,
        # we choose the one with minimal area
        # areas : [num_points, num_gts]
        # 将不在 gt 内部的点对应的 gt 的面积设为无穷大
        areas[inside_gt_bbox_mask == 0] = INF
        areas[inside_regress_range == 0] = INF
        min_area, min_area_inds = areas.min(dim=1)

        # labels 即每一个 point 对应的 label
        # min_area_inds : [num_points]
        # 此处的标签分配方法为:
        # (1) 对于每一个 point， 我们将其分配给它所在的若干个 gt中面积最小的那个
        # (2) 如果其不在任何一个 gt内, 则将其作为负样本(标签为 num_classes)
        # (3) 正样本的标签为其负责预测的 gt的类别标签([0, num_classes - 1])
        ...
        # global sum_pos
        # count = count + 1
        # sum_gt = sum_gt + num_gts
        # sum_pos = sum_pos + (labels != self.num_classes).sum(0).item()
        # with open("./fcos_coco.txt", "a") as file:
        #     file.write(f"---{count}---\n")
        #     file.write(f"{sum_pos / sum_gt}\n")
